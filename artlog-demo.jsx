...
// 1. ADMIN HOME
// ══════════════════════════════════════════════════════════════
const AdminHome=({students,schedules,notices,feedbacks,onAttendTap,onNavigate,attendanceMap,logoSrc,classTimes,isNativeApp,onExitApp})=>{
...
const StudentDetail=({student,feedbacks,artworks,academy,attendanceRecords=[],onBack,onFeedback,onManualFeedback,onEdit,onUpdateFeedback,onDeleteFeedback})=>{
  ...
  const tabs=[{id:"info",l:"기본정보"},{id:"artworks",l:`작품 ${arts.length}`},{id:"attendance",l:"출결"},{id:"feedback",l:"피드백"}];
  return(
    <div>
...
const SettingsPage=({onBack,initTab="academy",academy,onSaveAcademy,onNavigate,onLogout,onWithdraw,onExportData,studentCount})=>{
            ...
            <Card style={{background:`linear-gradient(135deg,${C.terra},${C.terraD})`}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                <div>
                  ...
                  <div style={{fontSize:18,fontWeight:800,color:"white"}}>Basic ₩9,900/월</div>
                  <div style={{fontSize:12,color:"rgba(255,255,255,0.8)",marginTop:4}}>학생 최대 20명 · 무제한 작품</div>
                </div>
                <button onClick={()=>showAlert("플랜 업그레이드는 준비 중입니다.")} style={{padding:"8px 14px",borderRadius:20,background:"rgba(255,255,255,0.2)",border:"1px solid rgba(255,255,255,0.4)",color:"white",fontSize:12,fontWeight:600,cursor:"pointer"}}>업그레이드</button>
              </div>
              <div style={{marginTop:12}}>
                <div style={{display:"flex",justifyContent:"space-between",marginBottom:4}}><span style={{fontSize:11,color:"rgba(255,255,255,0.75)"}}>학생</span><span style={{fontSize:11,color:"white",fontWeight:600}}>{studentCount} / 20명</span></div>
                <div style={{height:6,background:"rgba(255,255,255,0.2)",borderRadius:3}}>
                  <div style={{height:"100%",borderRadius:3,background:"white",width:`${Math.min(100,Math.round(studentCount/20*100))}%`}}/>
                </div>
              </div>
            </Card>
...
export default function App(){
  ...
  const onAddStudent = useCallback(async (s) => {
    try {
      ...
      alertMutationError(e, "학생 등록에 실패했습니다.");
      throw e;
    }
  }, [addStudent]);
  ...
  const renderAdmin=()=>{
    ...
    switch(adminTab){
      case"home":     return <AdminHome students={students} schedules={schedules} notices={notices} feedbacks={feedbacks} onAttendTap={setAttendSt} onNavigate={handleAdminNav} attendanceMap={attendMapDisplay} logoSrc={logoSrc} classTimes={classTimesForAttendance} isNativeApp={isNativeApp} onExitApp={handleExitApp}/>;
